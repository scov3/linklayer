...
export default function VaultPage() {
  ...
  const { currentVault, fetchVaultById, isCurrentVaultLoading, deleteVault } =
    useVaultStore();
  const {
    notes,
    currentNote,
  ...
  const { links, fetchLinks, createLink, deleteLink } = useLinksStore();
  ...
  useEffect(() => {
    if (vaultId) {
      ...
      void fetchLinks(vaultId).catch((error) => {
        console.error("[VaultPage] fetchLinks error:", error);
      });
    }
  }, [vaultId, fetchVaultById, fetchNotes, fetchLinks]);
  ...
  const vaultDescription =
    currentVault?.description || `Хранилище "${vaultTitle}"`;
          ...
          <div className="grid grid-cols-1 gap-6 xl:grid-cols-[380px_minmax(0,1fr)]">
            <Card>
              <CardContent className="p-4">
                {notesError && (
                  <p className="mb-3 text-sm text-destructive">{notesError}</p>
                )}
                {isNotesLoading ? (
            ...
            {editingNote ? (
              <NoteEditor
                ...
                onDelete={
                  editingNote.id
                    ? () => handleDeleteNote(editingNote.id as string)
                    : undefined
                }
                onRequestDelete={
                  editingNote.id
                    ? () => setNoteToDeleteId(editingNote.id as string)
                    : undefined
                }
              />
            ) : currentNote ? (
                ...
                <CardContent>
                  <div className="flex min-h-[420px] flex-col items-center justify-center text-center text-muted-foreground">
                    ...
                    <p className="mt-2 max-w-sm text-sm">
                      Редактор поддерживает Markdown и теги. Создайте заметку,
                      чтобы начать.
                    </p>
          ...
          <Card>
            ...
            <CardContent>
              <div className="text-center py-12 text-muted-foreground">
                ...
                <p className="mt-2">
                  Добавление и управление участниками хранилища
                </p>
              </div>
            </CardContent>
          </Card>
              ...
              <div className="space-y-4">
                ...
                <div className="flex justify-between items-center p-4 border border-destructive/30 rounded-md bg-destructive/5">
                  <div>
                    ...
                    <p className="text-sm text-muted-foreground">
                      Хранилище, заметки и связанные данные будут удалены без
                      восстановления.
                    </p>
                  </div>
                  <Button
