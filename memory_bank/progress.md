...
### 2026-04-17 - План граф-редактора и редактор заметок

#### Выполняется
- 🔄 DLV-005 уточнен как `CRUD и редактор заметок`, потому что пользовательский сценарий редактирования переносится в основную вкладку `Заметки`
- 🔄 Вкладка `Заметки` получает рабочий интерфейс: список заметок, создание черновика, редактирование Markdown, теги, просмотр и удаление
- ✅ Цветовая схема переводится с синего акцента на теплые бежевые оттенки
- ✅ Добавлены собственные адаптированные диалоги подтверждений, запросов и сообщений
- ✅ Browser `prompt/confirm/alert` удалены из пользовательских сценариев
- ✅ Добавлена кнопка удаления хранилища в настройках хранилища
- ✅ Убраны лишние `Загрузка...` из названия и настроек хранилища, где доступен fallback

#### План изменен
- DLV-008 теперь отвечает за редактор графа: ручное создание/удаление связей между заметками
- Физический режим графа вынесен в DLV-014 и должен строиться поверх уже готовой модели связей
- Проверка суммы весов deliverables: 5 + 8 + 7 + 8 + 8 + 9 + 8 + 8 + 7 + 7 + 7 + 4 + 10 + 8 = **100**
